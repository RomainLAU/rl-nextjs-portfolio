...
export default function PageTransition() {
    ...
    const [colorIndex, setColorIndex] = useState(Math.floor(Math.random() * RANDOM_COLORS.length))
    ...
    useEffect(() => {
        ...
        const handleRouteChangeComplete = () => {
            setTimeout(() => {
                ...
                const newColorIndex = Math.random() < 0.5 ? (colorIndex - 1 + RANDOM_COLORS.length) % RANDOM_COLORS.length : (colorIndex + 1) % RANDOM_COLORS.length
