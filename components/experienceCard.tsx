...
export default function ExperienceCard({
  ...
  const [, _setParagraphFragments] = useState<string[]>([]);
  ...
  const _isCompanyInView = useInView(companyRef, {
    once: isMobile,
    amount: 0.2,
  });
  ...
  useLayoutEffect(() => {
    if (paragraphRef.current) {
      ...
      const fragments = text
        ...
        .filter(Boolean);
      _setParagraphFragments(fragments);
    }
  ...
  const _renderFragment = (fragment: string, index: number) => {
  ...
  const renderTitleWord = (word: string, wordIndex: number) => {
    return (
      <m.span
        ...
        transition={{
          duration: 0.5,
          delay: index === 0 ? 1.2 + wordIndex * 0.3 : wordIndex * 0.3,
        }}
        className="block"
      >
        {word}
  ...
  return (
    <AnimatePresence>
      <m.div
        ref={ref}
        className="flex w-full flex-col gap-8 rounded-lg text-white shadow-md md:grid md:h-[calc(100dvh-12rem)] md:w-[70vw] md:grid-cols-2 md:grid-rows-[3fr_1fr] md:items-start md:gap-0 md:gap-y-4"
      >
        ...
        <m.h2
          ref={titleRef}
          className="max-w-min text-[3em] leading-none font-bold md:text-[6em]"
          style={{ wordSpacing: isMobile ? "" : "440px" }}
        >
          {experience.title.split(" ").map(renderTitleWord)}
        </m.h2>
        <div className="flex w-full flex-col justify-center gap-y-8 md:w-max">
          <m.p
            ref={dateRef}
            initial={{ opacity: 0, y: -30 }}
            animate={{
            ...
            className="font-[ui-monospace] text-[3em] leading-[normal] font-extrabold md:text-[calc(5dvh+4em)]"
          >
            {new Date(experience.started_at)
          ...
          >
            {(experience.finished_at &&
              new Date(experience.finished_at)
                .toLocaleDateString(locale)
                .replaceAll("-", "/")) ??
              "Present"}
          </m.p>
          ...
          {experience && !isMobile && (
            <m.div
              ...
              transition={{
                ...
                delay:
                  index === 0
                    ? 2.5 + experience.skills.length * 0.1
                    : 1.5 + experience.skills.length * 0.1,
              }}
              className="w-1/2"
              onClick={(e) => {
                e.preventDefault();
            ...
            >
              <LinkButton
                blank={false}
                link={``}
                text={locale === "fr" ? "En savoir plus" : "Learn more"}
              />
            </m.div>
          )}
        </div>
