...
export default function CustomCursor() {
  ...
  useEffect(() => {
    ...
    const getClassName = (target: HTMLElement) => {
      return typeof target.className === "string"
        ? target.className
        : (target.className as unknown as { baseVal: string }).baseVal;
    };
    ...
    const handleMouseOver = (event: MouseEvent) => {
      ...
      if (HOVER_ELEMENTS.includes(target.tagName)) {
        ...
        BLACKLIST_CLASSES.some((blacklisted) => className.includes(blacklisted))
      ) {
    ...
    const handleMouseOut = (event: MouseEvent) => {
      ...
      if (
        HOVER_ELEMENTS.includes(target.tagName) ||
        BLACKLIST_CLASSES.some((blacklisted) => className.includes(blacklisted))
      ) {
  ...
  return (
    <div
      ref={cursorRef}
      className="pointer-events-none fixed z-1000 hidden h-10 w-10 rounded-full mix-blend-difference sm:block"
      style={{
