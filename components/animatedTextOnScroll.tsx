...
export default function AnimatedTextOnScroll({
  text,
  customScroller,
  containerRef,
    ...
    const setupAnimation = () => {
      timelineRef.current = gsap.timeline({
        scrollTrigger: getScrollTriggerConfig(
          container,
          customScroller || window,
          isMobile
        ),
      });

      words.forEach((word, index) =>
        createWordAnimation(timelineRef.current!, word, index)
      );
    };
  ...
  return (
    <div
      ...
      className="text-xl leading-10 font-light tracking-widest text-white"
    >
